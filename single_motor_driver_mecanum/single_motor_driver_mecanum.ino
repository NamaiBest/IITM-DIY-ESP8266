...
const char* ssid = "IITM-DIY-Mecanum-1D";
...
<title>IITM DIY School Connect - Single Driver Mecanum</title>
<style>
:root {
  --ink:#172234;
  --muted:#607188;
...
body {
  ...
  padding: 16px;
  color: var(--ink);
  background: linear-gradient(145deg, #fdfefe 0%, #edf3fb 45%, #e3ecf8 100%);
...
.btn.stop {
  background: linear-gradient(135deg, #c2410c 0%, var(--brand-2) 100%);
  box-shadow: 0 7px 16px rgba(234, 88, 12, 0.3);
}
.fwd { grid-column: 2; }
.left { grid-column: 1; }
.stop { grid-column: 2; }
...
<main class="card">
  ...
  <p class="sub">ESP8266 Bot - Single Motor Driver Mecanum</p>
  <p class="note">Single driver mode supports normal drive controls.</p>
